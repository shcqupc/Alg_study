...
if __name__ == "__main__":
    ...
    myRatings = loadRatings("ml-1m/my-ratings.dat")
    ...
    predictions = bestModel.predictAll(candidates.map(lambda x: (1, x))).collect()
    recommendations = sorted(predictions, key=lambda x: x[2], reverse=True)[:50]

    print("Movies recommended for you:")
    #print("Movies number" + str(len(recommendations)))
    for i in range(len(recommendations)):
        print(recommendations[i][1])
        #print(str(str(i + 1) + movies[recommendations[i][1]]))
